...
(defn euler23 []
  (let [size 28213
        abundant (filter abundant? (range 1 size))
        sums-hash (into {} (map #(vector % true)  (distinct (for [x abundant, y abundant :while (and (<= y x) (<= (+ x y) size))] (+ x y)))))]
    (reduce + (for [i (range size) :when (not (get sums-hash i false))] i))))
